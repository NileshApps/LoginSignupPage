...
if (InVal != null){
	D = new Date(InVal);
	console.log(D);
	document.getElementById('in').innerHTML = get_time_str_without_sec2(D);	
}
var OutVal = window.localStorage.getItem(get_out_str());
if (OutVal != null){
	D = new Date(OutVal);
	console.log(D);
	document.getElementById('out').innerHTML = get_time_str_without_sec2(D);	
}
var TimeCompleted = window.localStorage.getItem(get_total_time_str());
console.log(TimeCompleted);
if(TimeCompleted!=null){
	document.getElementById('time-done').innerHTML = get_time_from_msec(TimeCompleted);		
}
else{
	document.getElementById('time-done').innerHTML = get_time_from_msec(0);	
}
...
if(val == null){		
	...
	document.getElementById('in').innerHTML = get_time_str_without_sec();	
}
}
function checkOut(){
...
if(val == null){	
	...
	document.getElementById('out').innerHTML = get_time_str_without_sec();
	document.getElementById('time-done').innerHTML = get_time_from_msec(diff);
	var WeekTime = window.localStorage.getItem(get_week_str());
	if(WeekTime == null)
